...
> 重读目的：不是重新找新知识，而是把第一次阅读时「读过但没吃透」的概念重新讲透，尤其是锁步、延迟隐藏这类硬件机制。
...
#### Q10：一个 warp 内的线程「近似锁步执行同一条指令，各自处理不同数据」是什么意思？

**答：** 锁步（lockstep）形容 32 个线程在同一个时钟节拍上、一起执行同一条指令，像军队齐步走一样踩在同一个点上。「处理不同数据」指指令虽然相同，但每个线程操作的是自己寄存器里的那份输入。以顶点着色器 `clipPos = mul(localPos, mvp)` 为例：一个 warp 的 32 个线程同时执行同一条 `mul`，但线程 0 用第 0 号顶点、线程 1 用第 1 号顶点……各算各的。
